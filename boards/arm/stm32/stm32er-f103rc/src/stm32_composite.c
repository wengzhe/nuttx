/****************************************************************************
 * boards/arm/stm32/stm32er-f103rc/src/stm32_composite.c
 *
...
int board_composite_initialize(int port)
{
  ...
  syslog(LOG_INFO, "Bind SPI to the MMC/SD driver, minor=0 slot=%d\n",
         STM32ER_F103RC_MMCSDSLOTNO);

  ret = mmcsd_spislotinitialize(0, STM32ER_F103RC_MMCSDSLOTNO, spi);
  if (ret != OK)
    {
      syslog(LOG_ERR,
        "ERROR: Failed to bind SPI port %d to MMC/SD minor=0 slot=%d %d\n",
         STM32ER_F103RC_MMCSDSPIPORTNO, STM32ER_F103RC_MMCSDSLOTNO,
         ret);
      return ret;
    }
