/****************************************************************************
 * boards/arm/stm32/stm32er-f103rc/src/stm32_usbmsc.c
 *
...
#ifdef CONFIG_ARCH_BOARD_STM32ER_F103RC
#  undef STM32ER_F103RC_MMCSDSLOTNO
#  define STM32ER_F103RC_MMCSDSLOTNO 0
#  undef STM32ER_F103RC_MMCSDSPIPORTNO
#  define STM32ER_F103RC_MMCSDSPIPORTNO 2
#else
...
int board_usbmsc_initialize(int port)
{
  ...
  syslog(LOG_INFO, "Initializing SPI port %d\n",
      STM32ER_F103RC_MMCSDSPIPORTNO);

  spi = stm32_spibus_initialize(STM32ER_F103RC_MMCSDSPIPORTNO);
  if (!spi)
    {
  ...
  syslog(LOG_INFO, "Bind SPI to the MMC/SD driver, minor=%d slot=%d\n",
      CONFIG_SYSTEM_USBMSC_DEVMINOR1, STM32ER_F103RC_MMCSDSLOTNO);

  ret = mmcsd_spislotinitialize(CONFIG_SYSTEM_USBMSC_DEVMINOR1,
                                STM32ER_F103RC_MMCSDSLOTNO, spi);
  if (ret < 0)
    {
      syslog(LOG_ERR,
         "ERROR: Failed to bind SPI port %d to MMC/SD minor=%d slot=%d %d\n",
         STM32ER_F103RC_MMCSDSPIPORTNO, CONFIG_SYSTEM_USBMSC_DEVMINOR1,
         STM32ER_F103RC_MMCSDSLOTNO, ret);
      return ret;
    }
