/****************************************************************************
 * boards/arm/stm32/doublecan/src/stm32_usbmsc.c
 *
...
#ifdef CONFIG_ARCH_BOARD_DOUBLECAN
#  undef DOUBLECAN_MMCSDSLOTNO
#  define DOUBLECAN_MMCSDSLOTNO 0
#  undef DOUBLECAN_MMCSDSPIPORTNO
#  define DOUBLECAN_MMCSDSPIPORTNO 2
#else
...
int board_usbmsc_initialize(int port)
{
  ...
  syslog(LOG_INFO, "Initializing SPI port %d\n",
      DOUBLECAN_MMCSDSPIPORTNO);

  spi = stm32_spibus_initialize(DOUBLECAN_MMCSDSPIPORTNO);
  if (!spi)
    {
  ...
  syslog(LOG_INFO, "Bind SPI to the MMC/SD driver, minor=%d slot=%d\n",
      CONFIG_SYSTEM_USBMSC_DEVMINOR1, DOUBLECAN_MMCSDSLOTNO);

  ret = mmcsd_spislotinitialize(CONFIG_SYSTEM_USBMSC_DEVMINOR1,
                                DOUBLECAN_MMCSDSLOTNO, spi);
  if (ret < 0)
    {
      syslog(LOG_ERR,
         "ERROR: Failed to bind SPI port %d to MMC/SD minor=%d slot=%d %d\n",
         DOUBLECAN_MMCSDSPIPORTNO, CONFIG_SYSTEM_USBMSC_DEVMINOR1,
         DOUBLECAN_MMCSDSLOTNO, ret);
      return ret;
    }
