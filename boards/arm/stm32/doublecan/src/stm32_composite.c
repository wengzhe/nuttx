/****************************************************************************
 * boards/arm/stm32/doublecan/src/stm32_composite.c
 *
...
int board_composite_initialize(int port)
{
  ...
  syslog(LOG_INFO, "Bind SPI to the MMC/SD driver, minor=0 slot=%d\n",
         DOUBLECAN_MMCSDSLOTNO);

  ret = mmcsd_spislotinitialize(0, DOUBLECAN_MMCSDSLOTNO, spi);
  if (ret != OK)
    {
      syslog(LOG_ERR,
        "ERROR: Failed to bind SPI port %d to MMC/SD minor=0 slot=%d %d\n",
         DOUBLECAN_MMCSDSPIPORTNO, DOUBLECAN_MMCSDSLOTNO,
         ret);
      return ret;
    }
